<template>
    <foreignObject v-bind="scale?board.attribute:expLoc" @click.stop="select"
                   :class="{'auto-event':checked,'no-event':!checked}"
    ...
    <g :transform="board.attribute.transform" v-show="scale&&propVue.element === board">
        <rect
                v-bind="border_rect"
                class="border-rect"
        />
        <!--        左上-->
        <circle
                :cx="board.attribute.x"
                :cy="board.attribute.y"
                class="nw-resize dot-gray"
                v-dot="'lt'"
        />
        <circle :cx="board.attribute.x" :cy="board.attribute.y" class="dot-white" />
        <line
                :x1="midpoint.x"
                :y1="board.attribute.y"
                :x2="midpoint.x"
                :y2="board.attribute.y - 20"
                stroke-width="2"
                stroke="black"
        />
        <!--        上-->
        <circle
                ...
                v-dot="'t'"
        />
        <circle :cx="midpoint.x" :cy="board.attribute.y" class="dot-white" />
        <!--        旋转点-->
        <circle
                :cx="midpoint.x"
                :cy="board.attribute.y - 20"
        ...
        />
        <!--        右上-->
        <circle :cx="brpoint.x" :cy="board.attribute.y" class="dot-gray" />
        <circle :cx="brpoint.x" :cy="board.attribute.y" class="dot-white" />
        <!--        左上-->
        <circle :cx="board.attribute.x" :cy="brpoint.y" class="dot-gray" />
        <circle :cx="board.attribute.x" :cy="brpoint.y" class="dot-white" />
        <!--        下-->
        <circle
                :cx="midpoint.x"
                :cy="brpoint.y"
...
<script setup>
    ...
    board.attribute.transform = computed(() => {
        if (!board.rotateAngle) {
            return "";
        }
        return `rotate(${board.rotateAngle},${midpoint.value.x},${midpoint.value.y})`;
    });
    const border_rect = computed(() => {
        const ele = props.board.attribute;
        const ans = {};
        ans.x = ele.x;
        ans.y = ele.y;
        ans.width = ele.width;
    ...
    board.COPY=()=>{
        let copy = _.omit(board, ['SAVEFUN','COPY'])
        copy=clone(copy)
